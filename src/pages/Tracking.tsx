...
const computeRouteStats = (points: HistoryPoint[]) => {
  if (points.length < 2) {
    return { distanceKm: 0, durationMin: 0, avgSpeed: 0, maxSpeed: 0 };
  }
  ...
  return { distanceKm: Math.round(dist * 10) / 10, durationMin, avgSpeed, maxSpeed: Math.round(maxSpeed) };
};

function buildDotIcon(color: string, size = 12) {
  const html = `<div style="width:${size}px;height:${size}px;border-radius:9999px;background:${color};box-shadow:0 10px 25px rgba(0,0,0,0.2),0 2px 6px rgba(0,0,0,0.25)"></div>`;
  return L.divIcon({ className: "traceur-dot", html, iconSize: [size, size], iconAnchor: [size / 2, size / 2] });
}
...
export default function Tracking() {
  ...
  const selectedVehicle = useMemo(() => vehicles.find((v) => v.id === vehicleId) || null, [vehicleId, vehicles]);
  ...
  const selectedDevice = useMemo(() => gpsDevices.find((d) => String(d.plate || "").trim() === selectedPlate) || null, [gpsDevices, selectedPlate]);

  const filteredVehicles = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return vehicles;
    return vehicles.filter((v) => {
      const label = `${v.marque || v.brand || ""} ${v.modele || v.model || ""} ${v.immatriculation || v.registration || ""}`.toLowerCase();
      return label.includes(q) || String(v.id).toLowerCase().includes(q);
    });
  }, [search, vehicles]);
  ...
  const syncGps = async () => {
    try {
      ...
      toast.success("GPSwox synchronisé");
    } catch {
  ...
  const drawRoute = (input: HistoryPoint[]) => {
    ...
    endMarkerRef.current = L.marker(coords[coords.length - 1], { icon: buildDotIcon("#111827", 14), zIndexOffset: 900 }).addTo(overlay);
    fitToRoute();
  };
  ...
  const loadHistory = async () => {
    ...
    if (!selectedDevice) {
      toast.error("Aucun GPSwox lié à cette plaque");
      return;
    }
    ...
    try {
      ...
      trackingService.setPositions(vehicleId, normalized);
      ...
      if (geofenceEnabled && geofenceCircleRef.current) {
        ...
        if (geofenceInsideRef.current === null) {
          ...
          toast(inside ? "Entrée dans la zone" : "Sortie de la zone", {
            description: selectedVehicle ? `${selectedVehicle.immatriculation || selectedVehicle.registration || ""}` : undefined
          });
        }
      }
  ...
  const setGeofenceAtCenter = () => {
    ...
    if (geofenceCircleRef.current) {
      ...
      geofenceCircleRef.current.setRadius(geofenceRadiusM);
      geofenceCircleRef.current.addTo(overlay);
    } else {
  ...
  const applyGeofenceVisibility = (enabled: boolean) => {
    ...
    if (geofenceCircleRef.current) geofenceCircleRef.current.addTo(overlay);
  };
    ...
    liveTimerRef.current = window.setInterval(async () => {
      try {
        ...
        setPoints((prev) => {
          ...
          trackingService.setPositions(vehicleId, updated);
          drawRoute(updated);
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-1">
              <CardTitle className="text-xl font-semibold">Tracking (Trajet vidéo)</CardTitle>
              <div className="text-xs text-muted-foreground">
                Sélectionnez un véhicule, chargez une période, puis lancez “Lecture” pour suivre le trajet en animation.
              </div>
            </div>
          ...
          <div className="rounded-3xl border bg-card overflow-hidden flex flex-col h-[52vh] lg:h-[calc(100dvh-16rem)]">
            <div className="p-4 border-b space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div className="space-y-1 min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground truncate">
                    {selectedVehicle ? `${selectedVehicle.immatriculation || selectedVehicle.registration || ""}` : "Aucun véhicule sélectionné"}
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Button size="sm" variant="outline" onClick={loadHistory} disabled={!vehicleId || !selectedDevice}>
                    Charger
                  </Button>
              ...
              <div className="space-y-2">
                <div className="text-xs font-medium">Véhicule</div>
                <div className="grid gap-2">
                  <div className="relative">
                    <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                    <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher véhicule..." className="pl-8" />
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <div className="text-muted-foreground truncate">
                      {selectedDevice ? `GPSwox: ${selectedDevice.plate || selectedDevice.name}` : "GPSwox: غير مرتبط"}
                    </div>
              ...
              <div className="rounded-2xl border bg-muted/40 p-3 space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <Button size="sm" variant="outline" onClick={setGeofenceAtCenter} disabled={!geofenceEnabled}>
                    Définir zone (centre de la carte)
                  </Button>
                </div>
              </div>
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button variant="outline" className="w-full" disabled={!vehicleId}>
                    Détails
                  </Button>
                </SheetTrigger>
                    ...
                    <div className="rounded-2xl border p-4 space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="font-semibold truncate">{selectedVehicle ? `${selectedVehicle.marque || selectedVehicle.brand || ""} ${selectedVehicle.modele || selectedVehicle.model || ""}` : "—"}</div>
                          <div className="text-sm text-muted-foreground truncate">{selectedPlate || "—"}</div>
                        </div>
                      ...
                      {selectedDevice ? (
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="space-y-1">
                              <div className="text-xs text-muted-foreground">GPSwox</div>
                              <div className="font-medium truncate">{selectedDevice.name}</div>
                            </div>
                            <div className="space-y-1">
                              <div className="text-xs text-muted-foreground">Vitesse (direct)</div>
                              <div className="font-medium">{Math.round(selectedDevice.speed || 0)} km/h</div>
                            </div>
                          </div>
                        </>
                      ) : null}
